...
namespace GCodeFromImage
{
    class VectorToGcode
    {
        ...
        public List<string> FinalGCode;
        public VectorToGcode(string filepath)
        {
            XDocument SVGFile = XDocument.Load(filepath);
            MemoryStream memoryStream = new MemoryStream();
            StreamWriter streamWriter = new StreamWriter(memoryStream);
            Parse(streamWriter, SVGFile);
        ...
        public static void Parse(StreamWriter writer, XDocument svgFile)
        {
        ...
        private static void elliptical_arcs(StreamWriter writer, double x1, double y1, double rx, double ry, double x_axis_rotation, double large_arc_flag, double sweep_flag, double x2, double y2)
        {
            ...
            double angle_to_end_point2 = (Math.PI / 2) - Math.Atan2(cx - x1, cy - y1);
            double angle_to_start_point2 = (Math.PI / 2) - Math.Atan2(cx - x2, cy - y2);

            writer.WriteLine(angle_to_start_point2 * (180 / Math.PI));
            writer.WriteLine(angle_to_end_point2 * (180 / Math.PI));
            writer.WriteLine("CX: " + cx + "; CY: " + cy);
            ...
            if (angle_to_end_point2 < 0 && angle_to_end_point2 > -(Math.PI / 2))
            {
                angle_to_end_point2 = Math.PI / 2 + angle_to_end_point2;
            }
            if (angle_to_end_point2 < 0 && angle_to_end_point2 < -(Math.PI / 2))
            {
                angle_to_end_point2 = (Math.PI * 2) - angle_to_end_point2 + (Math.PI / 2);
            }
            if (angle_to_end_point2 == -(Math.PI / 2))
            {
                angle_to_end_point2 = 0;
            }
            ...
            {
                for (double a = Math.Round(angle_to_start_point2, 2); a > Math.Round(angle_to_end_point2, 2); a -= 0.01)
                {
                    psi = (a * 3.14159f / 180.0f);
                    fi = Math.Atan2(rx * Math.Sin(a), ry * Math.Cos(a));
