...
namespace GCodeFromImage
{
    class VectorToGcode
    {
        ...
        private static void commands(List<double> path, StreamWriter writer, char cmd)
        {
            ...
            if (cmd is 'C')
            {
                if(path.Count() > 6)
                {
                    ...
                    {
                        bezier_curve(temp_x, temp_y, Convert.ToDouble(path[i]) - cX, mirror(path[i+1]) - cY, Convert.ToDouble(path[i+2]) - cX, mirror(path[i + 3]) - cY, Convert.ToDouble(path[i+4]) - cX, mirror(path[i+5]) - cY);
                ...
                {
                    bezier_curve(temp_x, temp_y, Convert.ToDouble(path[0]) - cX, mirror(path[1]) - cY, Convert.ToDouble(path[2]) - cX, mirror(path[3]) - cY, Convert.ToDouble(path[4]) - cX, mirror(path[5]) - cY);
            ...
            if (cmd is 'c')
            {
                if (path.Count() > 6)
                {
                    for (int i = 0; i < path.Count(); i += 6)
                    {
                        bezier_curve(temp_x, temp_y, temp_x + Convert.ToDouble(path[i]), temp_y + mirror(path[i+1]), temp_x + Convert.ToDouble(path[i+2]), temp_y + mirror(path[i+3]), temp_x + Convert.ToDouble(path[i+4]), temp_y + mirror(path[i+5]));
                ...
                {
                    bezier_curve(temp_x, temp_y, temp_x + Convert.ToDouble(path[0]), temp_y + mirror(path[1]), temp_x + Convert.ToDouble(path[2]), temp_y + mirror(path[3]), temp_x + Convert.ToDouble(path[4]), temp_y + mirror(path[5]));
            ...
            if (cmd is 'S')
            {
                bezier_curve(temp_x, temp_y, temp_x + (temp_x - temp_X1), temp_y + (temp_y - temp_Y2), Convert.ToDouble(path[0]) - cX, mirror(path[1]) - cY, Convert.ToDouble(path[2]) - cX, mirror(path[3]) - cY);
            ...
            if (cmd is 's')
            {
                ...
                bezier_curve(temp_x, temp_y, temp_x + (temp_x - temp_X1), temp_y + (temp_y - temp_Y2), temp_x + Convert.ToDouble(path[0]), temp_y + mirror(path[1]) , temp_x + Convert.ToDouble(path[2]) , temp_y + mirror(path[3]) );
            ...
            if(cmd is 'Q')
            {
                if(path.Count() > 4)
                {
                    ...
                    {
                        quadratic_bezier_curve(temp_x, temp_y, path[i], mirror(path[i+1]), path[i+2], mirror(path[i+3]));
                        temp_x = path[i+2];
                ...
                {
                    quadratic_bezier_curve(temp_x, temp_y, path[0], mirror(path[1]), path[2], mirror(path[3]));
                    temp_x = path[2];
            ...
            if(cmd is 'q')
            {
                if (path.Count() > 4)
                {
                    for (int i = 0; i < path.Count(); i += 4)
                    {
                        quadratic_bezier_curve(temp_x, temp_y, temp_x + path[i], temp_y + mirror(path[i+1]), temp_x + path[i+2], temp_y + mirror(path[i+3]));
                ...
                {
                    quadratic_bezier_curve(temp_x, temp_y, temp_x + path[0], temp_y + mirror(path[1]), temp_x + path[2], temp_y + mirror(path[3]));
            ...
            if(cmd is 'T')
            {
                quadratic_bezier_curve(temp_x, temp_y, temp_x + (temp_x - temp_XQ), temp_y + (temp_y - temp_YQ),  path[0], mirror(path[1]));
            ...
            if(cmd is 't')
            {
                quadratic_bezier_curve(temp_x, temp_y, temp_x + (temp_x - temp_XQ), temp_y + (temp_y - temp_YQ), temp_x + path[0], temp_y + mirror(path[1]));
                temp_x = temp_x + path[0];
                temp_y = temp_y + mirror(path[1]);
            ...
            if(cmd is 'A')
            {
                if(path.Count() > 7)
                {
                    for(int i = 0; i < path.Count(); i += 7)
                    {
                        elliptical_arcs(temp_x, temp_y, path[i] * (-1), path[i + 1], path[i + 2] * (-1), path[i + 3], path[i + 4], path[i + 5], mirror(path[i + 6]));
                        temp_x = path[i + 5];
                        temp_y = mirror(path[i + 6]);
                    }
                }
                else
                {
                    elliptical_arcs(temp_x, temp_y, path[0] * (-1), path[1], path[2] * (-1), path[3], path[4], path[5], mirror(path[6]));
                    temp_x = path[5];
            ...
            if(cmd is 'a')
            {
                if (path.Count() > 7)
                {
                    for (int i = 0; i < path.Count(); i += 7)
                    {
                        elliptical_arcs(temp_x, temp_y, path[i] * (-1), path[i + 1], path[i + 2] * (-1), path[i + 3], path[i + 4], temp_x + path[i + 5], temp_y + mirror(path[i + 6]));
                        temp_x = temp_x + path[i + 5];
                        temp_y = temp_y + mirror(path[i + 6]);
                    }
                }
                else
                {
                    elliptical_arcs(temp_x, temp_y, path[0] * (-1), path[1], path[2] * (-1), path[3], path[4], temp_x + path[5], temp_y + mirror(path[6]));
        ...
        private static void bezier_curve(double p0x, double p0y, double p1x, double p1y, double p2x, double p2y, double p3x, double p3y)
        {
            int subdivides = 10;
            double t = 0.0;
        ...
        private static void quadratic_bezier_curve(double p0x, double p0y, double p1x, double p1y, double p2x, double p2y)
        {
            int subdivides = 10;
            double t = 0.0;
        ...
        private static void circle(IEnumerable<XElement> circle_el)
        {
            foreach(XElement circle in circle_el)
            {
                ...
                for(int i=0; i < 360; i++)
                {
                    ...
                    double fi = Math.Atan2(r * Math.Sin(psi), r * Math.Cos(psi));
                    double xx = cx + (double)((r * Math.Cos(fi)));
                    double yy = cy + (double)(r * Math.Sin(fi));

                    x.Add(xx);
                    y.Add(yy);
                    if (i == 0) { x.Add(69421); y.Add(69421); }
                }
        ...
        private static void ellipse(IEnumerable<XElement> ellipse_el)
        {
        ...
        private static void rectangle(IEnumerable<XElement> rectangle_el)
        {
            double xx = 0;
            double yy = 0;
            double width;
            ...
            foreach (XElement rectangle in rectangle_el)
            {
                ...
                {
                    transform(rectangle.Attribute("transform").Value, startPointx, startPointy, secondPointx, secondPointy, thirdPointx, thirdPointy, fourthPointx, fourthPointy);
                }
                else
                {
                    x.Add(startPointx);
        ...
        private static void line(IEnumerable<XElement> line_el)
        {
        ...
        private static void polygon(IEnumerable<XElement> polygon_el)
        {
            string argSeparators = @"[\s,]|(?=(?<!e)-)";
        ...
        private static void elliptical_arcs(double x1, double y1, double rx, double ry, double x_axis_rotation, double large_arc_flag, double sweep_flag, double x2, double y2)
        {
            ...
            for(int i = 0; i < segments; i++)
            {
                ...
                bezier_curve(startX, startY, (startX + dx1), (startY + dy1), (endpointX + dx2), (endpointY + dy2), endpointX, endpointY);
        ...
        private static void polyline(IEnumerable<XElement> polyline_el)
        {
            string argSeparators = @"[\s,]|(?=(?<!e)-)";
        ...
        private static void transform(string attr, double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4)
        {
